...
# Strategy: Use small-gap donor (e.g. Phenothiazine/Phenoxazine) and strong acceptor (e.g. Triazine/Heptazine/N-substituted imides)
# Combination: Phenothiazine-Triazine derivative (PTZ-TRZ)
mol_id = "Red_TADF_PTZ_TRZ"
# Phenothiazine-Triazine dimer
smiles = "c1ccc2c(c1)Sc3ccccc3N2-c4nc(Cl)nc(Cl)n4"
...
if os.path.exists(f"{BASE}/xtbopt.xyz"):
    ...
    xyz_opt = xyz_init
    print(f"  [2/3] Step 2: Optimization failed, using initial XYZ.")
...
if "DATA:" in proc.stdout:
    ...
    print(f"  [3/3] TDDFT Failed: {proc.stderr}")
